...
try:
	...
	execfile("/home/pi/spn_controller/safteyAlarm.py")  				#Sets an safety alarm to 12 am
	...
	if script_locals["scriptReturn"] == 1:				#Mppt checks if the battery is okey
		...
		if float(script_locals["scriptReturn"]) <= 80.0:	#Checks if the surrounding temperature is under 80 degrees
			...
			execfile("/home/pi/spn_controller/measurement.py")		#Current Measurement (eg Node Current)
			sleep(90)												#Here should be placed execfile("/home/pi/spn_controller/howEverYouCallit.py")- python script or program for node communication
			#-----------------------------------------------------------------------------------------------------------------------------------
